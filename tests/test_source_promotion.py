...
class DummyStore:
    ...
    def create_source_proposal_decision(self, **kwargs):
...
def test_promote_creates_source_and_persists_decision() -> None:
    ...
    service = SourcePromotionService(session)  # type: ignore[arg-type]
    ...
    assert service.agent_run_store.calls[0]["decision_type"] == "promote"


def test_reject_persists_decision() -> None:
    service = SourcePromotionService(DummySession())  # type: ignore[arg-type]
    service.agent_run_store = DummyStore()
    ...
    assert len(service.agent_run_store.calls) == 1
    assert service.agent_run_store.calls[0]["decision_type"] == "reject"
